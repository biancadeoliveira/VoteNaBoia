...
namespace VoteNaBoia.DAL
{
    public class PeriodoRepository : IPeriodoRepository
    {
        ...
        public async Task<bool> IsPeriodoAbertoAsync(int IDTurma, int IDPeriodo)
        {
            var periodo = await _dbContext.Periodo.Where(x => x.IDTurma.Equals(IDTurma) && x.IDPeriodo.Equals(IDPeriodo)).FirstOrDefaultAsync();
